...
(function(){
  ...
  function movementList(name,direction){
    ...
    return '<div class="cash-movement-list">'+items.map(function(row){return '<div class="cash-movement-row" data-cash-ledger-id="'+esc(row.id)+'" data-cash-ledger-source="'+esc(row.source_key||'')+'"><div><b>'+esc(row.transaction_date)+'</b><small>'+esc(row.reference_number||row.notes||'No reference')+'</small></div><strong>'+peso(row.amount)+'</strong><span class="cash-movement-action"></span></div>'}).join('')+'</div>';
  }
  ...
  function render(){
    ...
    anchor.insertAdjacentElement('afterend',panel);
    document.dispatchEvent(new CustomEvent('bwc:cash-controls-rendered'));
  }
