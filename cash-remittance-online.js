...
(function(){
  ...
  async function load(){
    ...
    var results=await Promise.all([
      ...
      db.from('cash_transactions').select('id,transaction_date,cash_fund,direction,amount,source_key').eq('business_id',businessId).eq('branch_id',activeBranch()).order('transaction_date',{ascending:false})
    ]);
  ...
  function draw(){
    ...
    var current=rows.filter(period),pending=current.filter(function(row){return row.status==='Pending remittance'}),remitted=current.filter(function(row){return row.status==='Remitted'}),pendingTotal=pending.reduce(function(sum,row){return sum+Number(row.amount||0)},0),remittedTotal=remitted.reduce(function(sum,row){return sum+Number(row.amount||0)},0),expenseCash=cashRows.filter(function(row){return cashPeriod(row)&&isExpenseCashOut(row)}),cibExpense=expenseCash.filter(function(row){return row.cash_fund==='CIB'}).reduce(function(sum,row){return sum+Number(row.amount||0)},0),pettyExpense=expenseCash.filter(function(row){return row.cash_fund==='Petty Cash'}).reduce(function(sum,row){return sum+Number(row.amount||0)},0),box=document.getElementById('cashRemittanceCard');
