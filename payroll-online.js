...
(function(){
  ...
  async function syncAttendance(){
    ...
    try{
      var result=await db.from('payroll_attendance').select('id,worker_id,work_date,time_in,time_out,regular_hours,overtime_hours,regular_pay,overtime_pay,overtime_approved').eq('business_id',businessId()).eq('branch_id',branchId()).order('work_date',{ascending:false});
      if(result.error)throw result.error;
