...
(function(){
  ...
  function payrollSnapshot(data,workerId,start,end){
    var worker=(data.workers||[]).find(function(item){return item.id===workerId})||{},attendance=(data.attendance||[]).filter(function(item){return item.workerId===workerId&&inPayrollRange(item.date,start,end)}),jobs=(data.jobs||[]).filter(function(item){return item.workerId===workerId}),debts=(data.advances||[]).concat(data.loans||[]).filter(function(item){return item.workerId===workerId}),regular=attendance.reduce(function(sum,item){return sum+Number(item.regularPay||0)},0),overtime=attendance.filter(function(item){return item.overtimeApproved}).reduce(function(sum,item){return sum+Number(item.overtimePay||0)},0),jobPay=jobs.reduce(function(sum,job){return sum+(job.payments||[]).filter(function(payment){return inPayrollRange(payment.date,start,end)}).reduce(function(total,payment){return total+Number(payment.amount||0)},0)},0),deductions=debts.reduce(function(sum,debt){return sum+(debt.payments||[]).filter(function(payment){return inPayrollRange(payment.date,start,end)}).reduce(function(total,payment){return total+Number(payment.amount||0)},0)},0),gross=regular+overtime+jobPay;
    return {start:start,end:end,worker:{id:worker.id,name:worker.name||'Worker',code:worker.code||'',position:worker.position||'',type:worker.type||'',rate:Number(worker.rate||0)},jobs:jobs.map(function(job){var retention=Number(job.amount||0)*Number(job.retention||0)/100,paid=(job.payments||[]).filter(function(payment){return inPayrollRange(payment.date,start,end)}).reduce(function(sum,payment){return sum+Number(payment.amount||0)},0),allPaid=(job.payments||[]).reduce(function(sum,payment){return sum+Number(payment.amount||0)},0);return {number:job.number,vehicle:job.vehicle,plate:job.plate||'',work:job.work||'',amount:Number(job.amount||0),retention:retention,paid:paid,remaining:Math.max(0,Number(job.amount||0)-retention-allPaid)}}),debts:debts.map(function(debt){var paid=(debt.payments||[]).filter(function(payment){return inPayrollRange(payment.date,start,end)}).reduce(function(sum,payment){return sum+Number(payment.amount||0)},0),allPaid=(debt.payments||[]).reduce(function(sum,payment){return sum+Number(payment.amount||0)},0);return {reference:debt.reference||'',amount:Number(debt.amount||0),deducted:paid,balance:Math.max(0,Number(debt.amount||0)-allPaid)}}),daily:regular+overtime,jobWeek:jobPay,regularPay:regular,overtimePay:overtime,vehiclePay:jobPay,gross:gross,deductions:deductions,net:Math.max(0,gross-deductions),approvedAt:new Date().toISOString()}
  }
  ...
  async function syncPayrollArchive(){
    if(!online||!businessId())return;
    try{
      var periods=await db.from('payroll_periods').select('id,period_start,period_end,schedule_type,status,approved_at,issued_at').eq('business_id',businessId()).order('period_end',{ascending:false});if(periods.error)throw periods.error;
      var slips=await db.from('payslips').select('id,payroll_period_id,worker_id,payslip_number,gross_earnings,deductions,net_pay,status,approved_at,issued_at,snapshot').eq('business_id',businessId()).order('created_at',{ascending:false});if(slips.error)throw slips.error;
  ...
  async function issuePayroll(button){
    ...
    try{
      var issuedAt=new Date().toISOString(),slipResult=await db.from('payslips').update({status:'issued',issued_at:issuedAt}).eq('id',slip.id).eq('business_id',businessId()).select().single();if(slipResult.error)throw slipResult.error;
      var reference='PAYSLIP:'+slip.id,existingExpense=await db.from('expenses').select('id').eq('business_id',businessId()).eq('reference_number',reference).maybeSingle();if(existingExpense.error)throw existingExpense.error;if(!existingExpense.data){var worker=(data.workers||[]).find(function(item){return item.id===workerId})||{},expense=await db.from('expenses').insert({business_id:businessId(),expense_date:dates.end,supplier_name:null,receipt_number:null,category:'Salaries and Wages',description:'Issued payroll · '+(worker.name||'Worker')+' · '+slip.payslip_number,quantity:1,unit_amount:Number(slip.net_pay||0),payment_method:null,reference_number:reference,remarks:'Automatically posted when payslip was issued.',created_by:user.id});if(expense.error)throw expense.error}
      var all=await db.from('payslips').select('status').eq('business_id',businessId()).eq('payroll_period_id',period.id);if(all.error)throw all.error;var complete=(all.data||[]).length>0&&(all.data||[]).every(function(item){return item.status==='issued'}),periodResult=await db.from('payroll_periods').update(complete?{status:'issued',issued_at:issuedAt}:{status:'approved'}).eq('id',period.id).eq('business_id',businessId()).select().single();if(periodResult.error)throw periodResult.error;putArchive(data,periodResult.data,slipResult.data);mergeArchivedPayslips(data,archive(data).payslips);write(data);onlinePayrollStatus('Issued payslips are now posted securely to Expenses and Dashboard reporting.','info');decoratePayrollActions();alert('Payslip issued. The net pay is now recorded in Expenses and Dashboard reporting.');
  ...
  window.addEventListener('load',function(){setTimeout(function(){syncWorkers();syncAttendance();syncJobs();syncObligations();syncPayrollArchive()},120)});
