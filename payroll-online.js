...
(function(){
  ...
  async function syncPayrollArchive(){
    ...
    try{
      ...
      var slips=await db.from('payslips').select('id,payroll_period_id,worker_id,payslip_number,gross_earnings,deductions,net_pay,status,approved_at,issued_at,created_at,snapshot').eq('business_id',business).eq('branch_id',branch).order('created_at',{ascending:false});if(slips.error)throw slips.error;
      if(!scopeIsActive(scope))return;
