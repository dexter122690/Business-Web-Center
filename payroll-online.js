...
(function(){
  ...
  async function syncJobs(){
    ...
    try{
      ...
      setTimeout(decorateJobCompletionActions,0);
  ...
  async function completeJob(button){
    var data=read(),job=(data.jobs||[]).find(function(item){return item.id===button.dataset.id});
    if(!job||!job.online){alert('This vehicle job is not online yet. Save it online before marking the unit done.');return}
    if(job.status==='Completed'){alert('This unit is already marked done and is kept in the completed-units inventory archive.');return}
    if(!confirm('Mark '+job.vehicle+' as done? It will move to the completed-units archive in Inventory. Payment records remain available.'))return;
    button.disabled=true;var original=button.textContent;button.textContent='Saving...';
    try{
      var result=await db.from('payroll_vehicle_jobs').update({status:'Completed'}).eq('id',job.id).eq('business_id',businessId()).eq('branch_id',branchId()).select().single();
      if(result.error)throw result.error;
      Object.assign(job,localJob(result.data,job.payments));write(data);
      document.dispatchEvent(new Event('bwc:vehicle-jobs-loaded'));
      var tab=document.querySelector('[data-pr-tab="jobs"]');if(tab)tab.click();
      alert('Unit marked done. Open Inventory to see it in the completed-units archive with its material usage.');
    }catch(error){alert('The unit could not be marked done online. '+error.message);button.disabled=false;button.textContent=original}
  }
  function decorateJobCompletionActions(){
    var content=document.getElementById('prContent');if(!content||!/^Vehicle Jobs/.test((content.textContent||'').trim()))return;
    var data=read(),rows=content.querySelectorAll('table tbody tr');
    Array.prototype.forEach.call(rows,function(row){
      var first=row.querySelector('td'),action=row.lastElementChild;if(!first||!action||action.tagName!=='TD')return;
      var job=(data.jobs||[]).find(function(item){return String(first.textContent||'').indexOf(String(item.number||''))>=0});
      if(!job||action.querySelector('[data-pr-complete-job], [data-job-done-badge]'))return;
      var marker=document.createElement(job.status==='Completed'?'span':'button');
      if(job.status==='Completed'){marker.className='badge';marker.dataset.jobDoneBadge='1';marker.textContent='Unit done'}
      else{marker.type='button';marker.className='primary';marker.dataset.prCompleteJob=job.id;marker.textContent='Mark unit done'}
      action.appendChild(document.createTextNode(' '));action.appendChild(marker);
    });
  }
  ...
  function localArchivedPayslip(row){var snapshot=row.snapshot||{};return {id:row.id,number:row.payslip_number,payrollPeriodId:row.payroll_period_id||'',workerId:row.worker_id||'',status:row.status||'',created:String(row.created_at||'').slice(0,10)||dateToday(),issuedAt:row.issued_at||'',paymentSource:row.payment_source||'',start:snapshot.start||'',end:snapshot.end||'',worker:snapshot.worker||{},jobs:snapshot.jobs||[],debts:snapshot.debts||[],daily:Number(snapshot.daily||0),jobWeek:Number(snapshot.jobWeek||0),deductions:Number(row.deductions||snapshot.deductions||0),gross:Number(row.gross_earnings||snapshot.gross||0),net:Number(row.net_pay||snapshot.net||0),approved:row.status==='approved'||row.status==='issued',issued:row.status==='issued',online:true}}
  ...
  async function syncPayrollArchive(){
    ...
    try{
      ...
      var slips=await db.from('payslips').select('id,payroll_period_id,worker_id,payslip_number,gross_earnings,deductions,net_pay,status,approved_at,issued_at,payment_source,created_at,snapshot').eq('business_id',business).eq('branch_id',branch).order('created_at',{ascending:false});if(slips.error)throw slips.error;
      if(!scopeIsActive(scope))return;
      var data=read();data.onlinePayrollArchive={periods:periods.data||[],payslips:slips.data||[]};mergeArchivedPayslips(data,slips.data||[]);write(data);setTimeout(function(){decoratePayrollActions();renderIssuedPayslipArchive()},20);
    }catch(error){onlinePayrollStatus('Online payroll archive could not load: '+error.message,'error')}
  }
  function decoratePayrollActions(){
    var data=read(),dates=payrollDates(data),saved=archive(data);Array.prototype.forEach.call(document.querySelectorAll('[data-pr="approve-payslip"]'),function(button){var workerId=button.dataset.id,period=(saved.periods||[]).find(function(item){return item.period_start===dates.start&&item.period_end===dates.end}),slip=period&&(saved.payslips||[]).find(function(item){return item.payroll_period_id===period.id&&item.worker_id===workerId});if(!slip)return;if(slip.status==='issued'){button.textContent='Issued ✓';button.disabled=true;Array.prototype.forEach.call(button.parentNode.querySelectorAll('[data-pr-issue-payslip="'+workerId+'"],[data-pr-payment-source="'+workerId+'"]'),function(item){item.remove()});return}if(slip.status==='approved'){button.textContent='Approved ✓';var source=button.parentNode.querySelector('[data-pr-payment-source="'+workerId+'"]'),issue=button.parentNode.querySelector('[data-pr-issue-payslip="'+workerId+'"]');if(!source){source=document.createElement('select');source.dataset.prPaymentSource=workerId;source.title='Choose where this issued salary was paid from';source.innerHTML='<option value="">Paid from…</option><option value="CIB">CIB</option><option value="Petty Cash">Petty Cash</option><option value="Authorized Manager">Authorized Manager</option>';button.insertAdjacentText('afterend',' ');button.insertAdjacentElement('afterend',source)}if(!issue){issue=document.createElement('button');issue.type='button';issue.className='primary';issue.dataset.prIssuePayslip=workerId;issue.textContent='Issue';source.insertAdjacentText('afterend',' ');source.insertAdjacentElement('afterend',issue)}}});
  }
  function payrollText(value){return String(value==null?'':value).replace(/[&<>"']/g,function(character){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[character]})}
  function issuedPeriod(item,periods){var id=item.payrollPeriodId||item.payroll_period_id;return (periods||[]).find(function(period){return period.id===id})||null}
  function issuedDate(item,period){return String(item.issuedAt||item.issued_at||(period&&period.issued_at)||item.created||item.created_at||(period&&period.period_end)||item.end||'').slice(0,10)}
  function issuedWorker(item){var worker=item.worker||(item.snapshot&&item.snapshot.worker)||{};return worker.name||worker.full_name||'Worker'}
  function renderIssuedPayslipArchive(){
    var content=document.getElementById('prContent');if(!content)return;
    var archiveBox=document.getElementById('prIssuedPayslipArchive'),isSummary=/Weekly payroll preview|Weekly payroll summary|Gross earnings|Net payroll/.test(content.textContent||'');
    if(!isSummary){if(archiveBox)archiveBox.remove();return}
    var data=read(),saved=archive(data),filter=data.issuedPayslipArchiveFilter||{},periods=saved.periods||[],month=filter.month||'',year=filter.year||'',monthNames=['January','February','March','April','May','June','July','August','September','October','November','December'],items=(data.payslips||[]).filter(function(item){return item.issued===true||item.status==='issued'}).map(function(item){var period=issuedPeriod(item,periods),date=issuedDate(item,period);return {item:item,period:period,date:date}}),years=[];
    items.forEach(function(entry){var itemYear=entry.date.slice(0,4);if(itemYear&&years.indexOf(itemYear)<0)years.push(itemYear)});years.sort(function(a,b){return b.localeCompare(a)});
    var filtered=items.filter(function(entry){var date=entry.date;return (!month||date.slice(5,7)===month)&&(!year||date.slice(0,4)===year)}).sort(function(a,b){return b.date.localeCompare(a.date)});
    if(!archiveBox){archiveBox=document.createElement('section');archiveBox.id='prIssuedPayslipArchive';archiveBox.className='card';archiveBox.style.marginTop='16px';content.appendChild(archiveBox)}
    function selectOptions(values,current,labels){return values.map(function(value){return '<option value="'+value+'"'+(value===current?' selected':'')+'>'+payrollText(labels?labels(value):value)+'</option>'}).join('')}
    var monthValues=['01','02','03','04','05','06','07','08','09','10','11','12'],rows=filtered.map(function(entry){var item=entry.item,period=entry.period,start=item.start||(period&&period.period_start)||'',end=item.end||(period&&period.period_end)||'',range=start&&end?start+' to '+end:(start||end||'—');return '<tr><td>'+payrollText(entry.date||'—')+'</td><td>'+payrollText(item.number||'—')+'</td><td>'+payrollText(issuedWorker(item))+'</td><td>'+payrollText(item.paymentSource||item.payment_source||'Not recorded')+'</td><td>'+payrollText(range)+'</td><td>'+money(item.gross||item.gross_earnings||0)+'</td><td>'+money(item.deductions||0)+'</td><td><strong>'+money(item.net||item.net_pay||0)+'</strong></td></tr>'}).join('');
    archiveBox.innerHTML='<div class="heading" style="margin-top:0"><div><div class="k">Issued payslips archive</div><h2>Released payroll by month</h2><p class="muted">Only payslips marked Issued are stored here. Use the filters to review released payroll without mixing it with the current calculation.</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><label>Month <select data-pr-issued-filter="month"><option value="">All months</option>'+selectOptions(monthValues,month,function(value){return monthNames[Number(value)-1]})+'</select></label><label>Year <select data-pr-issued-filter="year"><option value="">All years</option>'+selectOptions(years,year)+'</select></label></div></div>'+(rows?'<div style="overflow:auto"><table><thead><tr><th>Issued date</th><th>Payslip no.</th><th>Employee</th><th>Paid from</th><th>Payroll period</th><th>Gross</th><th>Deductions</th><th>Net issued</th></tr></thead><tbody>'+rows+'</tbody></table></div>':'<div class="empty">No issued payslips for the selected month and year.</div>');
  }
  ...
  window.addEventListener('click',function(event){
    var button=event.target.closest('[data-pr="add-worker"], [data-worker-edit-save="1"], [data-pr="save-attendance"], [data-pr="add-job"], [data-pr="add-advances"], [data-pr="add-loans"], [data-pr="approve-payslip"], [data-pr-issue-payslip], [data-pr-payment="job"], [data-pr-payment="advance"], [data-pr-payment="loan"], [data-pr-edit="job"], [data-pr-edit="advance"], [data-pr-edit="loan"], [data-pr-history="advance"], [data-pr-history="loan"], [data-pr-complete-job], [data-pr-approve-ot], [data-pr-approve-attendance], [data-pr-add-adjustment], [data-pr-approve-adjustment]');if(!button||!online)return;
  ...
  document.addEventListener('change',function(event){var select=event.target.closest('[data-pr-issued-filter]');if(!select)return;var data=read(),filter=data.issuedPayslipArchiveFilter||{};filter[select.dataset.prIssuedFilter]=select.value;data.issuedPayslipArchiveFilter=filter;write(data);renderIssuedPayslipArchive()});
})();
