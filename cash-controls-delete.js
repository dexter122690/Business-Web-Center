...
(function(){
  ...
  async function reconcilePettyCash(){
    ...
    var note=inputValue('cashPettyReconcileNote');if(!note){alert('Enter the meeting note or reason first.');return}
    var movements=await db.from('cash_transactions').select('direction,amount').eq('business_id',businessId).eq('branch_id',branch()).eq('cash_account','Petty Cash');
    if(movements.error){alert('Petty Cash balance could not be checked: '+movements.error.message);return}
    var balance=(movements.data||[]).reduce(function(total,row){return total+(row.direction==='In'?1:-1)*(Number(row.amount)||0)},0),amount=Math.round(Math.abs(balance)*100)/100;
    if(amount<0.005){alert('Petty Cash is already zero.');return}
    var direction=balance>0?'Out':'In',description='Owner meeting reconciliation — '+note;
    if(!confirm('Record '+direction+' of PHP '+amount.toLocaleString('en-PH',{minimumFractionDigits:2})+' to set Petty Cash to zero? This is a cash reconciliation adjustment only, not an expense or sale.'))return;
