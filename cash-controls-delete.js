...
(function(){
  ...
  async function pettyExpense(){
    var description=inputValue('pettyExpenseDescription'),category=inputValue('pettyExpenseCategory')||'Other',amount=Number(inputValue('pettyExpenseAmount'))||0;
    if(!description||!amount){alert('Enter an expense description and amount first.');return}
    if(!confirm('Record this as an expense paid from Petty Cash?'))return;
    var date=new Date().toISOString().slice(0,10),expense=await db.from('expenses').insert({business_id:businessId,branch_id:branch(),expense_date:date,supplier_name:'Petty Cash',receipt_number:null,category:category,description:description,quantity:1,unit_amount:amount,payment_method:'Petty Cash',reference_number:'Petty Cash expense',remarks:'Quick petty cash expense entry',created_by:userId}).select('id').single();
    if(expense.error){alert('Expense could not be saved: '+expense.error.message);return}
    var cash=await db.from('cash_transactions').insert({business_id:businessId,branch_id:branch(),cash_account:'Petty Cash',direction:'Out',amount:amount,transaction_date:date,source_key:'petty-expense:'+expense.data.id,reference_number:'Petty Cash expense',notes:description,created_by:userId});
    if(cash.error){alert('Expense was saved, but the Petty Cash deduction could not be saved: '+cash.error.message);return}
    alert('Petty Cash expense recorded and balance updated.');var tab=document.querySelector('[data-t="expenses"]');if(tab)tab.click();document.dispatchEvent(new CustomEvent('bwc:cash-updated'));
  }
